...
class ProductRestControllerTest {
	...
	public void context() {
		...
		when(resttemplate.getForObject(COUPON_SERVICE_URL+COUPON_CODE, Coupon.class)).thenReturn(coupon, null);
		when(repo.save(product)).thenReturn(product);
		Product create = controller.create(product);
		assertEquals(COUPON_CODE,create.getCouponCode());
